...
import {
  useQuotation, useSaveQuotation, useDeleteQuotation, Quotation,
...
export default function QuotationBuilder() {
  ...
  useEffect(() => {
    if (data) {
      ...
      setHeader({
        brand_title: q.brand_title,
        film_format: q.film_format ?? '',
        shooting_days: q.shooting_days,
        quotation_date: q.quotation_date,
        agency_name: q.agency_name ?? '',
        product_name: q.product_name ?? '',
        film_title: q.film_title ?? '',
        master_film_duration: q.master_film_duration ?? '',
        shooting_city: q.shooting_city ?? '',
        number_of_shifts: q.number_of_shifts,
        language: q.language ?? 'Bangla',
        status: q.status,
        vat_applicable: q.vat_applicable,
      });
      const a: Record<string, number> = {};
      data.items.forEach((it) => { a[it.id] = Number(it.amount) || 0; });
      setAmounts(a);
    }
  }, [data]);
  ...
  if (!data) {
    return (
      <div className="py-20 text-center text-sm" style={{ color: 'var(--text-muted)' }}>
        Quotation not found.
        <button onClick={() => navigate('/quotations')} className="block mx-auto mt-3" style={{ color: 'var(--brand)' }}>← Back</button>
      </div>
    );
  }

  const { quotation, items } = data;
  const total = items.reduce((s, it) => s + (amounts[it.id] ?? 0), 0);
  ...
  const setAmount = (itemId: string, v: string) => {
    setAmounts((a) => ({ ...a, [itemId]: Number(v) || 0 }));
    setSaved(false);
  };
  ...
  async function doSave() {
    await save.mutateAsync({
      ...
      amounts: items.map((it) => ({ id: it.id, amount: amounts[it.id] ?? 0 })),
    });
    setSaved(true);
  }

  async function saveThenPrint() {
    if (canEdit) await doSave();
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {canEdit && (
            <button onClick={doSave} disabled={save.isPending} className="flex items-center gap-1.5 text-sm px-3.5 py-1.5 rounded-md text-white font-medium disabled:opacity-50" style={{ background: 'var(--brand)' }}>
      ...
      <Panel title="Budget Summary — Production Cost">
          ...
          <tbody>
            {items.map((it, i) => (
              <tr key={it.id} className="border-t" style={{ borderColor: 'var(--border)' }}>
                <td className="px-3.5 py-1.5 font-mono text-xs" style={{ color: 'var(--text-muted)' }}>{i + 1}</td>
                <td className="px-3.5 py-1.5">{it.category}</td>
                <td className="px-3.5 py-1.5 text-right">
                  <input
                    ...
                    className="w-40 rounded-md px-2.5 py-1.5 text-sm text-right outline-none font-mono"
                    style={inputStyle}
                  />
                </td>
              </tr>
        ...
        <p className="px-3.5 py-2 text-xs" style={{ color: 'var(--text-muted)' }}>
          Excluding TAX &amp; VAT{header.vat_applicable ? ' — VAT applies over and above this amount.' : ''}
        </p>
      </Panel>
      ...
      <div className="rounded-md border" style={{ borderColor: 'var(--border)' }}>
        ...
        {termsOpen && (
          <pre className="px-3.5 py-3 text-xs whitespace-pre-wrap font-sans" style={{ color: 'var(--text-secondary)' }}>{data.terms || 'No terms template set.'}</pre>
        )}
      </div>
