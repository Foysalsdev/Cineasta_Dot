...
import { LayoutDashboard, Clapperboard, Wallet, BarChart3, Settings } from 'lucide-react';
      ...
      <nav className="flex-1 px-2 py-2 space-y-0.5 overflow-y-auto">
        {NAV.filter((item) => hasPermission(item.module, 'can_view')).map((item) => (
          <NavLink key={item.to} to={item.to} end={item.to === '/'}
            ...
            style={({ isActive }) => ({ background: isActive ? 'var(--brand-soft)' : 'transparent', color: isActive ? 'var(--brand)' : 'var(--text-secondary)', fontWeight: isActive ? 600 : 400 })}>
            <item.icon size={15} />
            {item.label}
          </NavLink>
        ))}
